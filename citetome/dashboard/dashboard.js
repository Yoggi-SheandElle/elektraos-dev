const API = (typeof window !== "undefined" && (window.CITETOME_API || window.LOOMGRAPH_API)) || "";
const TENANT = "yossra";
const KEY_STORE = "citetome.key";
...
async function api(path, opts = {}) {
  ...
  const k = authKey();
  if (k) { headers["X-Loomgraph-Key"] = k; headers["X-Citetome-Key"] = k; }
...
function renderGlobe() {
  ...
  const hubPoint = { ...HUB, label: "api.citetome.com", isHub: true };
